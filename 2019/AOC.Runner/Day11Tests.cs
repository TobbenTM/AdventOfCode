...
namespace AOC.Runner
{
    public class Day11Tests
    {
        ...
        {
            var result = await Day11.Solve(_input, 0);
            Assert.Equal(2428, result.Values.Count);
        }

        [Fact]
        public async Task Part2()
        {
            var map = await Day11.Solve(_input, 1);
            var (maxX, maxY, minX, minY) = (map.Keys.Max(x => x.x), map.Keys.Max(x => x.y), map.Keys.Min(x => x.x), map.Keys.Min(x => x.y));
